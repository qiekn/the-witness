...
public:
    ...
    RandGrid() {
        ...
        colors = vector<EntityColor::Color>({EntityColor::RGB_RED, EntityColor::RGB_GREEN, EntityColor::RGB_BLUE, EntityColor::RGB_YELLOW, EntityColor::RGB_CYAN, EntityColor::RGB_MAGENTA, EntityColor::RGB_WHITE});
    ...
    Grid randStars() {
        ...
        vector<vector<std::shared_ptr<Entity>>> v = vector<vector<std::shared_ptr<Entity>>>(9, vector<std::shared_ptr<Entity>>(9));
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
    ...
    Grid randMaze() { // Generates a grid containing 12 cutoffs in (1<<16) attempts to get the edges.
        ...
        vector<vector<std::shared_ptr<Entity>>> v(9, vector<std::shared_ptr<Entity>>(9));
    ...
    Grid randBlobs(int numBlobs, int numCols, int numCuts) { // Generates a grid containing up to numBlobs blobs. This grid also has up to numCuts cuts.
        ...
        for (auto i : things) {
            ...
            v[i.first][i.second] = std::shared_ptr<Blob>(new Blob(((size_t)(perm[reg] % numCols) < colors.size()) ? colors[perm[reg] % numCols] : EntityColor::RGB_GREY));
        }
