...
public:
  ...
  RandGrid() {
    ...
    start = {8, 0}; // The grid is still double in size however points with both
                    // coordinates odd cannot be traversed.
    end = {0, 8};
  ...
  Grid randStars() {
    ...
    vector<vector<std::shared_ptr<Entity>>> v =
        vector<vector<std::shared_ptr<Entity>>>(
            9, vector<std::shared_ptr<Entity>>(9));
    for (int i = 0; i < 9; i++) {
      for (int j = 0; j < 9; j++) {
    ...
    for (auto i : ps) {
      if (index & 1)
        v[i.first * 2 + 1][i.second * 2 + 1] =
            std::shared_ptr<Star>(new Star(colors[0]));
      else
        v[i.first * 2 + 1][i.second * 2 + 1] =
            std::shared_ptr<Star>(new Star(colors[1]));
      index++;
    }
  ...
  Grid randMaze() { // Generates a grid containing 12 cutoffs in (1<<16)
                    // attempts to get the edges.
  ...
  int blobRegionScale(int x) { // Minimum number of regions in blobs as a
                               // function of the number of colors
    return (int)(x * log(x) / log(2));
  }

  int blobPathScale(int x) { return (12 + randint(8)) << 1; }

  int minRegionSize(int x) { return 16 / x; }

  Grid randBlobs(int numBlobs, int numCols,
                 int numCuts) { // Generates a grid containing up to numBlobs
                                // blobs. This grid also has up to numCuts cuts.
    ...
    while (attempts < NUM_ATTEMPTS) {
      ...
      if ((int)(gridRegions.size()) < blobRegionScale(numCols))
        continue;
      ...
      if (minSize < minRegionSize(gridRegions.size()))
        continue;
    ...
    for (int i = 0; (size_t)i < perm.size(); i++)
      perm[i] = i;
    ...
    while (
        (int)(things.size()) < numBlobs &&
        count <
            NUM_ATTEMPTS) { // Attempt to collect dots evenly from all regions
      int subcount = 0;
    ...
    for (auto i : things) {
      ...
      for (int index = 0; (size_t)index < gridRegions.size(); index++) {
        if (reg >= 0)
          break;
        if (gridRegions[index].find(i) != gridRegions[index].end())
          reg = index;
      }
  ...
  Grid randTriangles(int numTriangles,
                     int numCuts) { // Generates a grid with up to numTriangles
                                    // triangles and up to numCuts cuts
    set<pair<int, int>> path = possiblePaths[randint(possiblePaths.size())];
    ...
    for (auto i : things) {
      ...
      for (int d = 0; d < 4; d++) {
        ...
        if (path.find({xp, yp}) != path.end())
          count++;
      }
      if (count > 0)
        v[i.first][i.second] = std::shared_ptr<Triangle>(new Triangle(count));
    }
  ...
  Grid randDots(int numDots, int numCuts) { // Generates a grid with up to
                                            // numDots dots and numCuts cuts
    set<pair<int, int>> path = possiblePaths[randint(possiblePaths.size())];
    ...
    for (auto i : things)
      v[i.first][i.second] = std::shared_ptr<Dot>(new Dot());

    // Return the grid
    return Grid(v);
  }

  Grid randBlocks(int numBlocks,
                  int numCuts) { // Generates a grid with a single region
    ...
    while (attempts < NUM_ATTEMPTS) { // At most 2 blocks per region
      ...
      if (gridRegions.size() >= BLOCKS_REGIONS)
        break;
      attempts++;
    }
    ...
    vector<pair<int, int>>
        points; // Points in a region, compressed into block form
    ...
    vector<pair<pair<int, int>, BlockGroup *>>
        groupies; // List of blocks and their placements for the region
    ...
    for (auto p : gridpoints) {
      ...
      for (int i = 0; (size_t)i < thingsvec.size(); i++) {
        int dist = abs(thingsvec[i].first - p.first) +
                   abs(thingsvec[i].second - p.second);
        if (dist < mind) {
          mind = dist;
          mini = i;
        }
    ...
    std::shuffle(thingsvec.begin(), thingsvec.end(), g);
  ...
  Grid randChallengeBlocks(int numCuts) { // Generates a grid with a single
                                          // region partitioned into 2 blocks.
    ...
    while (true && count < (1 << 16)) {
      ...
      for (auto p : region) {
        if (p.first % 2 == 0 || p.second % 2 == 0)
          continue;
        if (instanceof<BlockGroup>(v[p.first][p.second]))
          continue;
        sz++;
      }
      if (sz < 2) {
    ...
    for (auto p : things)
      v[p.first][p.second] = std::shared_ptr<Star>(new Star(colors[1]));
    Grid grid = Grid(v);
    return grid;
  }

  Grid
  randChallengeStars(int numCuts) { // Generates a grid with up to 4 same color
                                    // stars, 4 dots, and up to numCuts cuts.
    // Get 2 colors
    std::shuffle(colors.begin(), colors.end(), g);
    ...
    while (attempts < NUM_ATTEMPTS) {
      ...
      if (gridRegions.size() <= STARS_REGIONS)
        continue;
      int valids = 0;
      for (auto region : gridRegions) {
        int count = 0;
        ...
        if (count >= 2)
          valids++; // Need at least 2 blocks per region
      }
      if (valids >= 2)
        break;
    }
    ...
    for (auto i : intset) {
      ...
      for (auto i : things)
        v[i.first][i.second] = std::shared_ptr<Star>(new Star(colors[0]));
    }
    ...
    for (auto p : things)
      v[p.first][p.second] = std::shared_ptr<Dot>(new Dot());
  ...
  void dfs(pair<int, int> src, pair<int, int> prev, int numPaths) {
    ...
    for (int ii = 0; ii < 4; ii++) {
      ...
      if (!inside(next))
        continue;
      if ((next.first % 2 == 1) && (next.second % 2 == 1))
        continue;
  ...
  void visualize(int x) {
    if (x < 0 || (size_t)x >= possiblePaths.size())
      return;
    getRegions(possiblePaths[x]);
    for (int i = 0; i < 9; i++) {
      for (int j = 0; j < 9; j++) {
        if (possiblePaths[x].find({i, j}) != possiblePaths[x].end())
          cout << "#";
        else {
          bool bad = true;
