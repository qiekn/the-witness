...
Grid::Grid(vector<vector<std::shared_ptr<Entity>>>& v) { // Once a grid is created it cannot be changed unless changes are consistent across all aspects.
  n = v.size();
  m = 0;
  ...
  board = vector<vector<std::shared_ptr<Entity>>>(n, vector<std::shared_ptr<Entity>>(m));
...
bool Grid::ver(int sx, int sy) {
  ...
  map<EntityColor::Color, vector<std::shared_ptr<Entity>>> ding; // List of colors
  ...
  for (auto ii : blobs) {
    ...
    while (q.size() > 0) {
      ...
      if (instanceof<Blob>(cur)) {
        if (ding.find(cur->color) == ding.end()) ding.insert({cur->color, vector<std::shared_ptr<Entity>>()});
    ...
    for (auto i : ding) {
      if (i.first != maxcolor && i.first != EntityColor::NIL) hasmorecolors = true;
    }

    // cout << "AND THE WINNING COLOR IS " << maxcolor << endl;
    for (auto i : collected) {
      if (board[i.first][i.second]->color != maxcolor && board[i.first][i.second]->color != EntityColor::NIL) violations.insert(i);
