...
inline Vector2 endpointdisplacement(Grid g, int rank, int file, double prodist = PRODIST) {
  if (rank == 0) return {0, static_cast<float>(-prodist)};
  else if (rank == g.n - 1) return {0, static_cast<float>(prodist)};
  else if (file == 0) return {static_cast<float>(-prodist), 0};
  else if (file == g.m - 1) return {static_cast<float>(prodist), 0};
  else return {0, 0};
  ...
  for (int y = 0; y < g.n; y++) {
    for (int x = 0; x < g.m; x++) {
      ...
      if (instanceof<Blob>(e)) {
        ...
        Rectangle rect = {static_cast<float>(POS.first - obj_width), static_cast<float>(POS.second - obj_width), static_cast<float>(2 * obj_width), static_cast<float>(2 * obj_width)};
    ...
    if (LOCKEDIN && (LATENCY == 0)) {
      ...
      for (int block = 0; block < blah; block++) {
        ...
        if (dreamscometrue);
          ...
          bool badtrace = false;
          // Path test
          Vector2 thevec = {static_cast<float>(newcurspos.x + 2 * sgn(md.x) * THICKNESS), newcurspos.y};
        ...
        // In a column but not an intersection
          ...
          bool badtrace = false;
          // Path test
          Vector2 thevec = {newcurspos.x, static_cast<float>(newcurspos.y + 2 * sgn(md.y) * THICKNESS)};
          for (int i = 0; i < pathpos.size() - 0.5 * SPACING; i++) {
            if (rsqvec2(thevec, pathpos[i]) <= 4 * INTERACTION_EPSILON * INTERACTION_EPSILON) badtrace = true;
          }
          // Cut test
