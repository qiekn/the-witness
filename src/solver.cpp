...
Solver::Solver() { solution = vector<pair<int, int>>(); }
...
void Solver::path(pair<int, int> src, pair<int, int> prev) {
  ...
  for (int ii = 0; ii < 4; ii++) {
    ...
    bool blocked1 =
        !grid.inside(x1) || grid.board[x1.first][x1.second]->isPathOccupied;
    // bool blocked2 = !grid.inside(x2);
    bool blocked3 =
        !grid.inside(x3) || grid.board[x3.first][x3.second]->isPathOccupied;
  ...
  for (int ii = 0; ii < 4; ii++) {
    ...
    if (!grid.inside(next))
      continue;
    if (!grid.board[next.first][next.second]->isPath)
      continue;
...
string Solver::to_string() {
  for (auto i : grid.board) {
    for (auto j : i)
      j->isPathOccupied = false;
  }
  for (auto i : solution)
    grid.board[i.first][i.second]->isPathOccupied = true;
  string res = grid.to_string();
  for (auto i : solution)
    grid.board[i.first][i.second]->isPathOccupied = false;
  return res;
}
